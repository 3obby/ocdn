...
| **Store-blind** | Three independent protections. (1) **Content-blind**: doubly-encrypted shards — convergent encryption (inner) wrapped by mint's random key M (outer). Stores cannot determine content identity. (2) **Operator-blind**: stores operate behind anonymous transport (Tor hidden service default). Relay-escrowed mappings point to opaque addresses, not physical operators. Content→shard→address is derivable (Argon2-gated); address→operator is cryptographically broken. (3) **Payment-blind**: store earnings paid as Cashu ecash tokens (Chaum blind signatures). Mint cannot link issuance to redemption. No Lightning address, no payment-layer deanonymization. Adversary can identify *what* is stored at an address but not *who* operates it or *where earnings flow*. Visibility proportional to responsibility: mints are visible (custodial), stores are invisible (commodity). |
...
**Anonymous transport boundary**: All store communication routes through anonymous transport (Tor by default). The user-facing path (client ↔ serve endpoint) is clearnet. The infrastructure path (serve endpoint ↔ stores, mint ↔ stores) is anonymous. Serve endpoints bridge the two — the only component that connects to both clearnet clients and anonymous stores.
...
1. **Relay-encrypted mappings + escrowed M (mandatory, durable, compute-gated)** — mints MUST publish on every store registration and periodically refresh for active stores: `encrypt(store_address || blob_id, key=Argon2id(content_hash || shard_index, "ocdn-discovery-v1", ARGON2_PARAMS))`. Store address is opaque (.onion by default) — relay mapping reveals retrieval path but not operator identity. Additionally, M is relay-escrowed: `encrypt(M, key=Argon2id(content_hash || "ocdn-key-v1", ARGON2_PARAMS))`. Compute-hard KDF: ~1-3s per shard on desktop, bulk scanning linearly expensive. No mint needed for decryption or M recovery. Survives mint exits/crashes — the defense against coordinated mint takedown (mints are the smallest, most identifiable, most regulatable actor set). Serve endpoints SHOULD re-publish on successful reconstruction (belt-and-suspenders). **Relay durability and store-blindness coexist**: the relay reveals content→address (needed for recovery), anonymous transport hides address→operator (needed for protection). Different links, independently defended.
...
- **Store liability isolation**: Unbonded, anonymous — no on-chain footprint, no network identity linking operator to protocol. Store-blind via two independent layers (see Glossary: Store-blind): content-blind (doubly-encrypted shards) + operator-blind (anonymous transport, .onion default). Relay-escrowed mappings point to opaque addresses, not physical operators. An adversary who derives the mapping (Argon2, ~1-3s) learns content→address; address→operator requires breaking anonymous transport. **Visibility proportional to responsibility**: mints are visible (custodial, bonded); stores are invisible (commodity, unbonded). Legal posture: operator cannot be identified, compelled, or served — not by analogy to Tor relay but literally behind anonymous transport. Blob-ID removal on valid legal order directed at the opaque address (safe harbor). Enforcement: challenge failure = no payment. Sybil (false redundancy) is a mint-policy / clearinghouse concern, not protocol-level. System heals through replacement — compliance and censorship-resistance are independent properties.
...
- Lightning node (CLN/LND) for inbound deposits + Cashu library for blind-signed store payouts
...
**Sats bind to hashes; four separated roles (store, serve, mint, genesis) settle at participant parity; the founder operates nothing; the income is settlement math; the moat is the economic engine.**
