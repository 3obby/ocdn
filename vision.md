...
| **Referrer witness** | Serve endpoint signs per-epoch, per-content referrer-bucketed proof counts. Cross-verifies mint's `referrer_root` claims. Serve endpoints have via-tag income incentive for accurate referrer accounting. |
...
**Consumption flow (healthy)**: (1) Client signs request proof via NIP-07 (includes `via` referrer tag + `epoch_hash` set by front-end). (2) Client sends request proof to serve endpoint (mandatory proxy — client never contacts mint or store directly). (3) Serve endpoint verifies `epoch_hash` against its own computation (first boundary check — wrong genesis pubkey rejected here). Forwards to mint; mint verifies epoch_hash + PoW + signature + epoch + coverage ≥ K. Mint selects K shards via deterministic shard selection (`shard_i = hash(epoch_hash || content_hash || request_proof_hash || i) mod N`, deduplicated — see Unresolved #29), then selects one store per shard via verifiable deterministic selection (see Glossary: Delivery token). Issues delivery tokens (blob_id + store endpoint + epoch + nonce + selection_proof + mint_sig — no content_hash) and a sealed key envelope (K_shard values + per-shard selection proofs, encrypted to client pubkey — see Glossary: Key envelope). (4) Serve endpoint presents delivery tokens to respective stores. (5) Store verifies epoch_hash + mint signature on delivery token (store cannot verify selection — store-blind, lacks content_hash), serves doubly-encrypted blob. (6) Store signs attestation referencing delivery token hash, submits to mint directly. (7) Serve endpoint returns doubly-encrypted blobs + sealed key envelope to client. (8) Client opens key envelope, **verifies per-shard selection proofs** against public `store_set_root` and deterministic selection formula (client knows all inputs — content_hash, epoch_hash, shard_index, request_proof_hash), strips mint encryption layer (AES decrypt with K_shard), strips convergent encryption layer (content key), reconstructs plaintext from K shards, verifies content_hash. Incorrect selection proof → client publishes evidence (self-contained, no key revelation needed) + routes away from mint. (9) Serve endpoint publishes a sample of forwarded request proofs to its relay at epoch end + random jitter (primary demand witness — serve endpoints have the data, the relay infrastructure, and via-tag income incentive for correct demand accounting). Clients may also self-publish with probability `1/RELAY_SAMPLE_RATE`, delayed to epoch boundary + uniform jitter in `[0, RELAY_JITTER_BLOCKS]`. Agents with identified keys publish at rate 1. At bootstrap (`RELAY_SAMPLE_RATE = 1`), every proof is published — identical to the pre-scale behavior. (9b) Serve endpoint publishes per-epoch referrer witness event — compact per-content referrer-bucketed proof counts, signed by serve endpoint, cross-verifiable against mint's `referrer_root` (see Events: Referrer witness).
...
| Mint-store collusion (biased routing) | Three-layer detection: (1) **Client** verifies selection proofs in key envelope against deterministic formula + committed `store_set_root` — mint cannot forge Merkle proofs. (2) **Settler** cross-references attestation store pubkeys against expected selection from public data — actual routing mismatch caught ex-post. (3) **Store** compares observed delivery token count against committed demand data (`routing_root`) — aggregate under-routing detected per-epoch. Store cannot verify individual selection (store-blind — lacks content_hash), but the client (who has all inputs) is the strongest adversarial verifier. Without this: vertically integrated mint captures ~10× revenue (coordination + all storage shares) invisibly. Residual: selective dropping (see Unresolved #29). |
...
**Residual**: (a) selective dropping — mint doesn't issue delivery tokens for requests selecting unwanted stores (see #29). (b) Tenure amplification — biased routing gives mint's stores perfect tenure while honest stores get sporadic selection → lower tenure → voluntary exit (see #30). (c) Serve endpoint selection pattern observation (mitigated by request_proof_hash variance). (d) Mid-epoch store joins (aligns with tenure ramp). (e) Key envelope size (~6KB for K=10 at tree depth 20).
...
### 28. Split-Deposit M Generation Race Condition

When deposits split across 2+ mints (DEPOSIT_SPLIT_MIN), upload verification (Step 1b) and M generation must happen at exactly one mint — otherwise both mints independently generate different M values for the same content_hash, producing inconsistent outer encryption layers. Stores acquiring shards wrapped under M_A can't be served through Mint_B (which would issue key envelopes using M_B).

**Likely fix**: client designates one "verifying mint" for upload (Step 1b + M generation). Secondary mints accept sats-only deposits. Secondary mint confirms only after detecting the verifying mint's fund confirmation on relays (proves verification passed, M exists in gossip/relay escrow). HTLC timeout accommodates propagation delay; if verifying mint fails, all mints reclaim via HTLC.

**Open questions**: (i) Should secondary mints verify shards independently? Probably not — garbage content with no valid shards at secondary mints is self-correcting (no stores appear, pool sweeps). Upload verification at one mint is sufficient anti-griefing. (ii) Fund confirmation event should indicate verifying vs. secondary mint role (or at minimum, secondary mints should reference the verifying mint's confirmation event). (iii) If verifying mint goes down after generating M but before gossip completes, relay-escrowed M is the recovery path — but the escrow must happen atomically with (or before) the fund confirmation. (iv) The HTLC timeout chain: primary verification + relay propagation + secondary confirmation must fit within the funder's HTLC window — needs concrete timing analysis.

### 29. Selective Dropping and Shard Selection

Two residual routing-bias attacks that selection verification does not address:

**(i) Selective dropping**: The mint can refuse to issue delivery tokens for requests whose deterministic selection would route to unwanted stores. The selection function is unbiasable (all inputs committed or client-generated), so the mint's only remaining degree of freedom is *not executing* the selection. The mint drops requests that would select honest stores, processes only requests selecting its own stores. Every issued delivery token is correctly selected — no forged proofs, no detectable selection error.

**Detection**: Client verification (key envelope proofs) does not catch this — no token is issued, so no proof to verify. Settler detects ex-post: total attestations at mint M for content X vs. total request proofs for X on relays — a mint processing only 20% of valid request proofs has a conspicuous gap. `routing_root` demand commitment lets stores compare observed delivery tokens against committed totals (see #20). Cross-mint comparison is strongest: same content at two mints with divergent throughput ratios. **Residual at bootstrap**: with 1 mint, no cross-mint comparison. Settler detection is the primary defense; store aggregate monitoring is the secondary.

**(ii) Shard selection (which K of N)**: For N=20, K=10, each retrieval needs K shards. Which K? Currently unspecified. A biased mint could always select shards where its own stores are present. Stores for unselected shards never earn attestations, never earn, and leave — reducing redundancy to exactly K. **Likely fix**: deterministic shard selection from public inputs: `shard_i = hash(epoch_hash || content_hash || request_proof_hash || i) mod N` (deduplicate collisions by incrementing). Client verifies via key envelope (same mechanism as store selection — knows all inputs). Over many requests with varying request_proof_hash, all N shards are selected, distributing attestations across the full shard set.

### 30. Tenure Amplification of Biased Routing

Biased routing compounds via the tenure mechanism. A mint that routes to its own stores gives those stores 100% attestation rate → perfect tenure → weight approaches 1.0. Honest stores at the same mint receive sporadic delivery tokens → intermittent attestation → tenure oscillates → lower effective earnings. Lower earnings → honest store's rebalance daemon ranks these shards lower → evicts them → mint's stores become a larger fraction → bias becomes the equilibrium.

The honest stores exit *on their own economic logic* — the rebalance daemon correctly calculates that shards at this mint aren't earning well. Competitive exit punishes the honest stores, not the biased mint. After several epochs, only the mint's stores remain, earning full tenure-weighted payouts. S_s decline is visible in epoch summaries but ambiguous (could be normal content interest waning).

**Partial mitigations**: (i) Tenure could decay gradually on missed epochs (`tenure = max(0, tenure - 1)`) rather than resetting to 0, making sporadic selection less punishing. (ii) Store daemon could track delivery token frequency per blob and flag discrepancies against pushed `earnings_rate` — if earnings_rate says "this blob is profitable" but delivery tokens are sparse, the signal is contradictory. (iii) `routing_root` demand commitment (#20) gives stores a verifiable signal independent of the mint's earnings_rate push. (iv) Settler publishes "routing anomaly" events for mints with request-proof-to-attestation ratio outliers — stores subscribe and factor into mint reputation.

**Fundamental tension**: store-blindness prevents the store from verifying per-blob demand against public data (would require content_hash). Stores detect *aggregate* mint-level bias and *temporal* per-blob anomalies. Per-blob absolute demand verification is delegated to settlers (who can cross-reference relay mappings → group_ids → routing_root → request proofs).

### 31. Bootstrap Routing Vulnerability

At bootstrap (1 mint, 1 serve endpoint, few stores), all routing-bias detection layers are weak: client evidence may not propagate (1 serve endpoint could suppress), settler may not be independent (founder runs it), cross-mint comparison impossible (1 mint), store statistical detection lacks data (few blobs, few epochs). A vertically integrated founder-mint can bias routing with limited detection.

**Honest framing**: the protocol is designed to be robust at scale; bootstrap requires trust in the founder's good faith, bounded by (a) the small seed budget (100-200K sats), (b) independent stores joining dilutes any bias, (c) bias becomes detectable as the store and mint count grows. Analogous to Bitcoin's bootstrap: early mining was centralizable; the protocol became robust as mining distributed. The seed budget IS the upper bound on bootstrap routing manipulation damage.
...
| M6 | Issue delivery token with biased store selection | ⊘ | [D] Client verifies selection proofs in sealed key envelope against deterministic formula + committed store_set_root (client has all inputs: content_hash, epoch_hash, shard_index, request_proof_hash). Settler audits actual routing by cross-referencing attestation pubkeys against expected selection. Store detects aggregate under-routing via routing_root demand commitment. Store cannot verify individual selection (store-blind — lacks content_hash). | [C] Client publishes self-contained evidence of incorrect selection (Merkle proof mismatch). Settler flags routing anomaly. Stores exit on aggregate signal. |
