...
**Phase 2a behavior:** Shares on disk, transient M in RAM. Serving mint reconstructs M from T peer shares on demand (first request per epoch per content pays ~1-2s Tor latency for share collection; subsequent requests use epoch-cached M). Key envelope generation is unchanged — client is unaware of threshold machinery. Security improvement: disk seizure reveals shares (information-theoretically zero bits of M for k < T), not full M. If committee members report divergent VSS commitments (PSS split), the serving mint groups shares by commitment set and reconstructs from the largest consistent group (≥ T members). Reads succeed through transient splits.
...
**Security property (Phase 2a — honest statement):** The emergency blob explicitly trades absolute threshold secrecy for recoverability. The protocol's security is bounded: disk seizure of fewer than T mints reveals zero information-theoretic bits of M; bulk recovery from relay escrow is economically prohibitive (~30-60s per item); but any party with protocol_seed can recover M for a specific content_hash given sufficient compute. This is defense-in-depth atop anonymous transport, not an absolute cryptographic wall. The strongest achievable online property within OCDN's single-mint-in-the-loop architecture: "no mint holds M unless it actively self-deals, at PoW cost per content per epoch, producing detectable commitment anomalies."

**Phase 2b (client-side reconstruction) — research direction, not planned phase:** Three known impossibilities bound its design space: (i) the serving mint can always be its own client at PoW cost, making "no mint ever holds M" unachievable within single-mint-in-the-loop architecture — the coordinator IS the man-in-the-middle by architectural necessity, and the committee cannot authenticate the client's identity independently; (ii) per-request committee interaction scales as O(reads × T) with no caching — incompatible with ephemeral client keys and uncompensated committee work at scale; (iii) per-shard K_shard_i sharing (required for multiplicative sub-linearity) makes M unrecoverable for re-upload without an emergency blob, collapsing Phase 2b's secrecy claim. Per-shard multiplicative sub-linearity can be explored within Phase 2a's trust model (serving mint reconstructs per-shard K_shard_i, then discards) without requiring Phase 2b's unachievable absolute secrecy property.
