...
| **Degraded-mode retrieval** | Fallback when no bonded mint is reachable. Stores accept PoW-authorized direct requests (blob_id + PoW proof, no delivery token). Serve endpoints discover stores via relay-encrypted mappings (Argon2-gated with `protocol_seed` in salt — requires correct genesis pubkey). The delivery token's authorization function is redundant with PoW; the mint adds mediation (routing, settlement). Mint liveness controls economics, not availability. Activates when store's persistent mint connections are down > MAX_SILENT_EPOCHS — locally verifiable, unfakeable by external parties. No settlement, no attestation, no income for any participant. Structurally worse for everyone → no perverse incentive. No genesis-agnostic fallback — degraded mode requires the correct genesis pubkey for discovery. |
...
6. **Store-blind storage** — Three independent protections (see Glossary: Store-blind): addressing-blind (random blob IDs, mapping mint-held), operator-blind (anonymous transport), payment-blind (Cashu). Stores cannot determine what they hold; no adversary can determine who operates a store or where earnings flow.
   ...
   **Legal posture**: Convergent-encrypted blob cache behind anonymous transport — cannot identify content (no content_hash on disk), cannot be identified as operator. Literally a Tor hidden service. Blob-ID removal on valid legal order directed at the opaque address (safe harbor). Operator's binary choice: store everything the daemon selects, or don't run it. Strictly stronger than BitTorrent (plaintext, identifiable peers, 20+ years of legal survival).
...
**Phase 2 — Reveal + Delivery (on dwell trigger)**: (5) Client reveals `selection_nonce` → serve endpoint → mint. Mint verifies `SHA256(nonce) == blind`. (6) Mint selects K shards + one store per shard via deterministic formulas against committed `store_set_root` (see Glossary: Delivery token). Issues delivery tokens + selection proofs. (7) Serve endpoint fetches convergent-encrypted blobs from stores using delivery tokens. Stores verify epoch_hash + mint signature, serve blobs, attest to mint directly. (8) Serve endpoint returns blobs + selection proofs to client. Client derives convergent key from content_hash + protocol_seed, verifies selection proofs, decrypts, reconstructs plaintext, verifies content_hash. Incorrect selection → publishable evidence. (9) Serve endpoint publishes sampled request proofs + referrer witness event at epoch end (see Events: Referrer witness).
...
5. **Fossil**: No stores. Bytes gone (see Glossary: Ghost). Economic fossil persists on relays (per-mint state roots); Bitcoin anchors by any interested party add permanent durability. Convergent encryption is deterministic — anyone with the original file can re-encrypt and re-upload. `[+] to restore`.
...
**Security model without M**: Store-blindness rests on two independent layers: (1) addressing-blind — blob_ids are random, the content_hash→blob_id mapping is mint-held; an adversary needs to compromise a mint to learn the mapping. (2) operator-blind — anonymous transport hides who operates the store; an adversary needs to compromise Tor to identify operators. Neither gives the other. This is strictly stronger than BitTorrent (plaintext, identifiable peers, no encryption, 20+ years of legal survival). The convergent encryption layer remains (shards are ciphertext at rest), but the key is derivable from content_hash + protocol_seed — a verification property, not a secrecy property.
...
| M6 | Issue delivery token with biased store selection | ⊘ | [D] Three-layer verification (see Glossary: Delivery token): client checks selection proofs, settler audits routing, store detects aggregate anomaly via routing_root. | [C] Client publishes evidence. Settler flags. Stores exit on aggregate signal. |
