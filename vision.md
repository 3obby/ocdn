...
| **Store-blind** | Stores hold doubly-encrypted shards — convergent encryption (inner, deterministic, for verification) wrapped by the mint's random encryption layer (outer, for store privacy). Stores cannot determine content identity. Adversary cannot prove content identity without M (held by bonded mints across jurisdictions, relay-escrowed behind Argon2). Tor-relay posture: encrypted blobs in, encrypted blobs out. |
| **Delivery token** | Mint-signed, single-use, epoch-bound authorization for a serve endpoint to fetch one blob from one store. Contains blob_id, store endpoint, epoch, nonce, mint signature. No content_hash. Replaces direct client-to-store discovery. |
...
6. **Store-blind storage** — Stores hold doubly-encrypted, erasure-coded shards under random blob IDs. They cannot determine what content their fragments represent, and this cannot be proven externally without keys held by bonded mints across jurisdictions.

   **How it works**: Two encryption layers. Inner: convergent encryption (deterministic from content — used by mints for upload verification). Outer: mint's random encryption layer (per-content key M, generated at upload, applied before distribution to stores). Stores receive and hold only the doubly-encrypted output. Blind addressing breaks the public content_hash → store link. Clients never contact stores directly — serve endpoints proxy all retrieval via mint-issued delivery tokens (see Glossary: Delivery token, Key envelope).

   **Privacy boundary**: Store-blindness protects against **bulk scanning** (PoW cost scales linearly with scan breadth) AND **targeted identification** — an adversary who possesses the plaintext can derive the convergent shard, but cannot match it against store disk contents without M (random, per-content, held by bonded mints across jurisdictions, relay-escrowed behind Argon2). Clients cannot identify stores (serve endpoint proxying). Serve endpoints see partial store mappings (one random store per shard per request) but not content identity (key envelope is opaque). Censorship resistance compounds: redundancy + jurisdiction diversity + economic replacement + cryptographic unlinkability.

   **Store posture**: Attestations reference delivery token hashes (see Glossary: Store attestation) — stores never appear in public events by real pubkey. Zero editorial decisions: software selects shards by economic signal, not by content. Storage is a commodity: encrypted blobs in, encrypted blobs out. Store daemon requires no convergent encryption logic and no RS WASM — just holds blobs, responds to challenges, attests.

   **Legal posture**: Doubly-encrypted blob cache — cannot decrypt, cannot be proven to hold specific content without cooperation of bonded mints in other jurisdictions. Equivalent to Tor relay: encrypted traffic in, encrypted traffic out. Complies with blob-ID removal on valid legal order.

   **Censorship resistance**: Requires taking down more than N-K stores across jurisdictions simultaneously, while economic incentives actively recruit replacements. The adversary's takedown action increases per-store payout, advertising the opportunity to replacement stores. Additionally, the adversary must obtain M from threshold-shared mints to even identify which stores to target — compounding the cost. The system channels self-interest (stores want sats, funders want permanence, readers want signal) into collective censorship resistance.
...
SERVE ENDPOINT ←────────→ STORES           fetches blobs using delivery tokens (no content_hash)
...
STORES ─────────────────→ MINT             attestations + mapping registration (private, references delivery token hash)
MINT   ←───────────────→ STORES            storage challenges (direct, out-of-band)
MINT   ←───────────────→ MINT              mapping gossip + M distribution (cross-mint replication)
MINT   ─────────────────→ RELAYS           coverage signals, relay-escrowed M + store locations (Argon2-gated)
```
...
1. **Relay-encrypted mappings + escrowed M (mandatory, durable, compute-gated)** — mints MUST publish on every store registration and periodically refresh for active stores: `encrypt(store_endpoint || blob_id, key=Argon2id(content_hash || shard_index, "ocdn-discovery-v1", ARGON2_PARAMS))`. Additionally, M is relay-escrowed: `encrypt(M, key=Argon2id(content_hash || "ocdn-key-v1", ARGON2_PARAMS))`. Compute-hard KDF: ~1-3s per shard on desktop, bulk scanning linearly expensive. No mint needed for decryption or M recovery. Survives mint exits/crashes — the defense against coordinated mint takedown (mints are the smallest, most identifiable, most regulatable actor set). Serve endpoints SHOULD re-publish on successful reconstruction (belt-and-suspenders).
...
- **Store liability isolation**: Unbonded — no on-chain footprint linking operator to protocol. Store-blind with Tor-relay posture (see Glossary: Store-blind). Disk contents are doubly encrypted — cannot be linked to specific content without M (held by bonded mints across jurisdictions, relay-escrowed behind Argon2). Network traffic indistinguishable from any Blossom blob server. Legal posture: doubly-encrypted blob cache — cannot decrypt, cannot be proven to hold specific content without cooperation of bonded mints in other jurisdictions. Blob-ID removal on valid legal order (safe harbor). Enforcement: challenge failure = no payment. Sybil (false redundancy) is a mint-policy / clearinghouse concern, not protocol-level. System heals through replacement — compliance and censorship-resistance are independent properties.
...
**Resolution**: Argon2id KDF for layer 1 mapping encryption. Key = `Argon2id(content_hash || shard_index, "ocdn-discovery-v1", ARGON2_PARAMS)`. Bulk scanning linearly expensive. Single-item lookup: ~1-3s (tolerable — layer 1 only fires when all mints are down). Privacy claim now consistent across all layers: "PoW cost scales linearly with scan breadth."

**Why layer 1 is non-negotiable**: Mints are the smallest, most identifiable, most regulatable actor set (on-chain bonds, public endpoints, custodial = money transmission). Coordinated mint takedown is the obvious state-actor playbook. Layer 1 decouples content availability from mint liveness — mint death becomes economic pause, not censorship. Eliminating layer 1 creates a single targetable bottleneck. The adversary analysis killed options (a) and (b).
...
### 6. Lightning Payout Failure Handling

Lightning payments fail routinely (routing failures, capacity). What happens when a payout fails? Retry policy, accumulation timeout. **Timed-out accumulated balances MUST NOT sweep to genesis** — same perverse incentive as bond slash (genesis profits from system malfunction). Failed payouts should accumulate and retry indefinitely, or until the store/mint provides an updated Lightning endpoint. Needs operational spec.
