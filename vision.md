...
| **Mint encryption layer** | Per-content random key M generated by the verifying mint at upload (single-writer — exactly one mint generates M per content_hash; see #28). `stored_shard_i = AES(convergent_shard_i, KDF(M, shard_index))`. M relay-escrowed with Argon2 gating before fund confirmation (escrow-before-confirmation atomicity), distributed to bonded mints via gossip as optimization. Any bonded mint can acquire M from relay escrow at any time. Stores never learn M or K_shard. Adversary cannot link store disk contents to specific content without M. |
