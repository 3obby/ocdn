...
| **Mint encryption layer** | Per-content random key M generated by the verifying mint at upload (single-writer — exactly one mint generates M per content_hash; see #28). `stored_shard_i = AES(convergent_shard_i, KDF(M, shard_index))`. M relay-escrowed with Argon2 gating before fund confirmation (escrow-before-confirmation atomicity). v1: distributed to bonded mints via gossip; any bonded mint can acquire M from relay escrow. v2 (threshold hardening, see #9): Shamir T-of-N shares via Feldman VSS, per-content committees of N_MAX (15) mints, proactive secret sharing (PSS) refresh every epoch. PSS convergence is leaderless — committee members publish VERIFY messages to relays, divergent commitment sets resolve via deterministic tiebreaker (`lower H(commitment_set)` wins), splits heal via catch-up protocol. No distinguished coordinator — the relay layer is the coordination substrate. Mandatory emergency blob (hard-Argon2 encrypted M) preserves availability when < T committee mints are reachable. Stores never learn M or K_shard. Adversary cannot link store disk contents to specific content without M. |
...
| **Store-blind** | Four independent protections: (1) **Content-blind** — doubly-encrypted shards (convergent inner + mint's M outer); stores cannot determine content identity. (2) **Operator-blind** — anonymous transport (.onion default); address→operator cryptographically broken. (3) **Payment-blind** — Cashu ecash (Chaum blind signatures); mint cannot link issuance to redemption. (4) **Hash-blind local state** — daemon persists only `blob_id → encrypted bytes` + `blob_id → earnings_rate`; no content_hash on disk. Seizure reveals opaque blobs under random IDs with no link to content hashes or operator identity. See Core Thesis #6, `ocdn-store` daemon. |
| **Delivery token** | Mint-signed, single-use, epoch-bound authorization for a serve endpoint to fetch one blob from one store. Contains blob_id, store address (opaque), epoch, epoch_hash, nonce, selection_proof, mint signature. No content_hash. **Serve-blinded verifiable store selection**: `selected_index = hash(epoch_hash \|\| content_hash \|\| shard_index \|\| request_proof_hash \|\| selection_nonce) mod S_s` against committed `store_set_root`. Selection nonce is client-generated, withheld until mint commits (see §3 Consumption flow). Mint cannot bias selection (nonce unknown at commitment time), cannot reject without evidence (signed processing_commitment). **Three-layer verification**: (1) client checks key envelope Merkle proofs (knows all inputs), (2) settler audits routing ex-post, (3) store detects aggregate anomalies via `routing_root`. Bundles authorization (anti-abuse, redundant with PoW) + mediation (per-request privacy). In degraded mode, stores accept PoW directly (see Glossary: Degraded-mode retrieval). |
...
| **Degraded-mode retrieval** | Fallback when no bonded mint is reachable. Stores accept PoW-authorized direct requests (blob_id + PoW proof, no delivery token). Serve endpoints discover stores via relay-encrypted mappings (Argon2-gated with `protocol_seed` in salt — requires correct genesis pubkey), recover M from relay escrow (also protocol_seed-gated). Under v1: M recovered via standard Argon2 (~1-3s). Under v2 (threshold M): M recovered from mandatory emergency blob via hard Argon2 (`HARD_ARGON2_PARAMS`: 4 GiB memory, 10 iterations — ~30-60s per item, bulk recovery prohibitive). The delivery token's authorization function is redundant with PoW; the mint adds mediation (per-request privacy, routing, settlement). Decomposing the two means mint liveness controls economics, not availability — the mandatory emergency blob preserves this principle under threshold M. Activates when store's persistent mint connections are down > MAX_SILENT_EPOCHS — locally verifiable, unfakeable by external parties. No settlement, no attestation, no income for any participant. Structurally worse for everyone → no perverse incentive. No genesis-agnostic fallback — degraded mode requires the correct genesis pubkey for discovery. **Privacy degradation is severe**: serve endpoint has M (from relay escrow or emergency blob recovery) AND can compute convergent key (knows content_hash + protocol_seed) — full plaintext access to all proxied content. See §3 consumption flow (degraded), Unresolved #14. |
...
MINT   ←───────────────→ MINT              mapping gossip + threshold share distribution + PSS sub-shares (per-content committees)
MINT   ─────────────────→ RELAYS           coverage signals, relay-escrowed threshold shares + emergency blob + store addresses (Argon2-gated)
...
| **`ocdn-store` daemon** | Product | Commodity storage behind anonymous transport. `docker run ocdn-store --disk 50GB`. Bundles Tor, binds .onion on first run. **Autonomous rebalance** (~1h): ranks shards by `projected_value_per_byte` (pool decay + tenure ramp + competitor entry), acquires via replication tokens, evicts lowest-value. Challenge-based tenure = implicit switching cost (held shards rank higher than new). Hash-blind local state (see Glossary: Store-blind). Earns Cashu ecash (auto-diversified across mints). Degraded mode: accepts PoW-authorized requests when all mints down (serves blob, earns nothing). Target: <512MB RAM, <100MB/day bandwidth. See Symbiosis for uptime economics. |
...
**Phase 2 — Reveal + Delivery (on dwell trigger)**: (5) Client reveals `selection_nonce` → serve endpoint → mint. Mint verifies `SHA256(nonce) == blind`. (6) Mint selects K shards + one store per shard via deterministic formulas against committed `store_set_root` (see Glossary: Delivery token). Issues delivery tokens + sealed key envelope (K_shard values + selection proofs, encrypted to client pubkey). (7) Serve endpoint fetches blobs from stores using delivery tokens. Stores verify epoch_hash + mint signature, serve blobs, attest to mint directly. (8) Serve endpoint returns blobs + envelope to client. Client opens envelope, verifies selection proofs, strips both encryption layers, reconstructs plaintext, verifies content_hash. Incorrect selection → publishable evidence. (9) Serve endpoint publishes sampled request proofs + referrer witness event at epoch end (see Events: Referrer witness).
...
**Consumption flow (degraded)**: See Glossary: Degraded-mode retrieval. Serve endpoint recovers mappings + M from relay escrow (Argon2-gated), contacts stores directly with PoW authorization. No settlement, no per-request privacy, serve endpoint has full plaintext access. Content available; economics and content confidentiality suspended.
...
- Content state tree (two tiers): **Per-mint state root** (`content_state_root`) = Merkle root over `protocol_seed || per-content economic states`. Closed per-mint computation — no cross-mint join. Stores verify content states against their mint's root; auditors verify custody. **Global state root** is an index-layer product (materializer aggregates per-mint data; deterministic, multiple indexes converge). **Bitcoin anchoring** is a market activity: any party anchors `"OCDN"(4B) || genesis_fingerprint(8B) || state_root(32B) || epoch(4B)` via OP_RETURN for their own reasons. Self-contained, gap-tolerant, non-conflicting by construction. Ghost state provable against per-mint roots on relays without any anchor.
...
| HARD_ARGON2_PARAMS | Inscription body | t=10, m=4GiB, p=4. Emergency blob KDF for threshold M relay escrow. ~30-60s recovery per item on commodity hardware; bulk recovery prohibitive. |
| EPOCH_BLOCKS | Inscription body | 24 (~4h). |
| RING_CONFIRM_DEPTH | Inscription body | 6. Reorg-proof. |
| N_MAX | Inscription body | 15. Maximum committee size for threshold M. |
...
M generated at upload by the verifying mint (single-writer — see #28), relay-escrowed **before** the fund confirmation is published (escrow-before-confirmation atomicity: if a consumer can see any fund confirmation, M is guaranteed on relays). **M persists indefinitely** — relay-escrowed M cannot be deleted (Nostr events are permanent and distributed). Ghost state is economic death, not cryptographic death: stores evict shards (no economic incentive), but M on relays enables recovery from any surviving offline copy. Re-funding reuses existing M from relay escrow — no new M generated, no VM/SM coordination needed (`role=additional`). If no shards survive, re-upload to a new VM which recovers M from relay escrow (emergency blob Argon2 recovery), re-encrypts, re-distributes.

**v1 (ship first, deprecated at threshold activation):** M distributed to bonded mints via authenticated gossip; any bonded mint can acquire M from relay escrow by computing `Argon2id(content_hash || protocol_seed || "ocdn-key-v1")`. Fund confirmations include `["m_escrow", event_id]`. Security property: any single compromised mint reveals M for content it knows about.

**Threshold M (Phase 2a — the target):** Shamir T-of-N threshold sharing with Feldman VSS over secp256k1, activated when the bonded mint set reaches N ≥ 3. Per-content committees of N_MAX (15) mints selected deterministically: `top N_MAX by H(content_hash || mint_pubkey || "ocdn-committee-v1")`. T = max(2, ceil((N+1)/2)) — simple majority. With 3 bonded mints: N=3, T=2. With 100 bonded mints: N=15, T=8. Evaluation points bind shares to mint identity AND content: `x_i = H(mint_pubkey_i || content_hash || "ocdn-share-eval-v1") mod q`. VSS commitments `C_j = a_j * G` (public, C_0 = M*G) enable share verification without revealing M.

**Phase 2a behavior:** Shares on disk, ephemeral M in RAM. Serving mint reconstructs M from T peer shares on demand (first request per epoch per content pays ~1-2s Tor latency for share collection), derives K_shard_i = KDF(M, shard_index) for the K selected shards, **discards M immediately**, and caches only the derived K_shard_i values. Subsequent requests reuse cached K_shard_i; uncached shard selections (new shard index not yet seen this epoch) trigger re-reconstruction from committee shares (~O(ln(N/K)) additional reconstructions per content per epoch, amortized across requests). M exposure window: microseconds (KDF evaluation time), not epoch-length. Key envelope generation is unchanged — client is unaware of threshold machinery. Security improvement: disk seizure reveals shares (information-theoretically zero bits of M for k < T), not full M; RAM snapshot outside the KDF window reveals only specific K_shard_i values (sufficient for content reconstruction via K shards, but not for deriving keys for non-selected shards or re-encrypting for re-upload — KDF is one-way). If committee members report divergent VSS commitments (PSS split), the serving mint groups shares by commitment set and reconstructs from the largest consistent group (≥ T members). Reads succeed through transient splits.

**No retroactive migration:** Existing v1 content retains its `m_escrow` format. Full M is already on relay escrow and in gossip history; re-sharing cannot revoke prior exposure. v1 and threshold content coexist indefinitely — the fund confirmation's escrow tag (`m_escrow` vs `m_escrow_v2`) is the version discriminator.

**Proactive secret sharing (PSS) — leaderless:** Every epoch, committee members refresh the polynomial (each generates a random degree-(T-1) polynomial with zero constant term, distributes sub-shares, accumulates). M is invariant (zero constant term preserves a_0). Old shares become incompatible with new shares — an adversary who compromises different mints in different epochs learns nothing (information-theoretic: 2k equations for 2T-1 unknowns, underdetermined when k < T). Collusion window bounded to one epoch (~4h). Refresh cost at 1M content, 100 mints: ~5-10 minutes, ~550 MB outbound per mint per epoch — comfortably within the 4h budget.

**PSS convergence model:** Committee members publish VERIFY messages as signed Nostr events to relays — each VERIFY declares which peers' sub-shares passed VSS verification. Each member independently computes the verified set (intersection of received VERIFYs) from relay-observed events. Divergent commitment sets (from relay propagation delays) resolve via deterministic tiebreaker: members adopt the commitment set with the lower `H(commitment_set)`, catching up from T peers on the canonical polynomial via re-sharing. Convergence is eventual (1-2 epochs), not synchronous — relay eventual consistency is the coordination substrate, not a distinguished leader. No blessed mint set, no leader election, no consensus protocol. Splits are tolerable during convergence (serving mint groups-and-reconstructs at read time) and self-healing (deterministic tiebreaker + catch-up). At bootstrap (few relays), the founder's serve endpoint relay archive is the de facto canonical relay for PSS events — a dependency that dilutes organically as more serve endpoints launch relay archives.

**Universal polynomial rejected:** Deriving M from a single shared master secret (M = KDF(S, content_hash)) would reduce PSS to O(1) per epoch, but is fatally incompatible: (a) all-or-nothing risk profile — compromising S maps everything, not proportional degradation; (b) forward secrecy failure — compromise at T0 derives M for content uploaded at T1 > T0; (c) adds threshold interaction to the upload hot path.

**Relay escrow (v2):** Write-once. Contains epoch-0 VSS commitments + per-mint encrypted shares (NIP-44 sealed) + **mandatory emergency blob** (`AES-256-GCM(M, key=Argon2id(content_hash || protocol_seed || "ocdn-key-v2-emergency", HARD_ARGON2_PARAMS))`). The emergency blob is mandatory, not optional — without it, re-upload after partial shard loss requires a new M and full shard re-distribution (KDF from M to K_shard is not invertible; M must be recoverable). `HARD_ARGON2_PARAMS` (4 GiB memory, 10 iterations, ~30-60s per item) make bulk recovery prohibitive while preserving per-item recoverability. Fund confirmations include `["m_escrow_v2", event_id]`. Not updated by PSS. Current-epoch VSS commitments published via `vss_root` in epoch summaries (Merkle root over per-content commitment tuples `(content_hash, epoch, T, C_0..C_{T-1})`, leaves queryable on demand from committee members).

**Security property (Phase 2a — honest statement):** The emergency blob explicitly trades absolute threshold secrecy for recoverability. The protocol's security is bounded: disk seizure of fewer than T mints reveals zero information-theoretic bits of M; bulk recovery from relay escrow is economically prohibitive (~30-60s per item); but any party with protocol_seed can recover M for a specific content_hash given sufficient compute. This is defense-in-depth atop anonymous transport, not an absolute cryptographic wall. The strongest achievable online property within OCDN's single-mint-in-the-loop architecture: "no mint holds M beyond microseconds of KDF evaluation; cached K_shard_i values are content-reconstructing but non-transferable (cannot derive unchosen shard keys, cannot re-encrypt for re-upload). Self-dealing requires PoW cost per content per epoch."

**Phase 2b (client-side reconstruction) — closed, impossibility triangle:** Three properties form an impossible triangle: (a) mint ignorance (mint never learns K_shard_i in cleartext), (b) epoch caching (first request pays committee cost, subsequent are local), (c) no per-request committee interaction. Any two are achievable; all three are not. (a)+(b) requires committee help per-request to transform cached ciphertext to client-specific output — violates (c). (a)+(c) requires mint to operate locally without reading keys — impossible since the envelope requires K_shard_i values or committee-produced ciphertexts. (b)+(c) is Phase 2a — violates (a). Explored constructions: proxy re-encryption fails (ephemeral client keys force per-request re-encryption), MPC for KDF evaluation fails (SHA-256 circuit depth × Tor latency ≈ minutes per shard), additive homomorphic encryption over Shamir shares works cryptographically but requires per-request committee interaction (the encryption targets the ephemeral client pubkey). Full per-shard K_shard_i sharing (Shamir-share each K_shard_i independently instead of M) is feasible but not worth the complexity: K cached shard keys are K-sufficient for content reconstruction (K = RS_K = reconstruction threshold), so information exposure at request time is equivalent to M-level sharing, while PSS refresh cost multiplies by N (~20×). Self-dealing (impossibility (i)) is not cryptographically detectable — the protocol path is identical for honest service and self-dealing; economic deterrents (bonding, PoW cost) are the correct layer. **Resolution**: ephemeral-M caching within Phase 2a (see Phase 2a behavior above) is the correct realization of per-shard multiplicative sub-linearity — M discarded after KDF evaluation, only K_shard_i values cached, zero PSS cost increase, zero protocol changes.

**Key rotation interaction (see #17):** Share evaluation points bind to mint_pubkey. Key rotation invalidates all shares — the rotating mint re-onboards under its new key via `KeyHandoff` event (old key signs endorsement of new key, `handoff_epoch >= current_epoch + 1`). No reputation transfer. Re-onboarding is lazy (on first request per content) via catch-up from T existing committee members. Consistent with #17: key loss is costly by design.

**Residual concerns**: (i) Relay pruning of escrowed M events — periodic heartbeat re-publication of identical escrow events (idempotent, does not contradict write-once — content is unchanged, distribution is refreshed). (ii) PSS split rate at bootstrap — founder's serve endpoint relay archive is the de facto coordination substrate for PSS VERIFY events; dilutes as serve endpoint relay archives multiply. Adversarial relay partitioning can create persistent splits; the system tolerates this as long as catch-up capacity exceeds split rate. (iii) Emergency blob Argon2 params calibration — 4 GiB may exceed low-end VPS RAM; params should target "expensive but possible on commodity hardware." (iv) Store-blindness anchors on anonymous transport (address→operator), not M secrecy (content→address). Threshold M strengthens content-blindness from a computational speed bump to an information-theoretic wall (for k < T compromised mints) but does not change the fundamental security model. (v) Text content (N=1) has no multiplicative sub-linearity — single committee, single shard. Marginal cost is zero (same threshold infrastructure), transport anonymity is the primary protection, M secrecy is defense-in-depth. (vi) Dome milestone: PSS is leaderless, so keystone removal simplifies to N - 1 ≥ T with the founder excluded AND the founder's relay archive is not the sole relay for PSS events. (vii) Write-once relay escrow creates an unbounded window for private-key-accumulation attacks (adversary collecting T mint identity keys over time can reconstruct from epoch-0 shares regardless of PSS). Acceptable because compromising T independent mint identity keys is catastrophic regardless — the accumulation attack is strictly weaker than real-time access to T operational mints. Escrow refresh on committee changes as future hardening.
...
**Stores cannot verify selection correctness.** This is an information-theoretic constraint: the property that protects stores (content-hash blindness) prevents them from verifying routing fairness. Three-layer verification compensates: (i) client verifies via key envelope Merkle proofs (real-time, all inputs known), (ii) settler audits routing ex-post (cross-references attestations against deterministic selection), (iii) store detects aggregate anomalies via `routing_root` demand commitment (statistical, per-epoch). See Glossary: Delivery token, Key envelope.

**Store-blind demand detection**: stores detect *aggregate* mint-level bias (delivery token frequency vs epoch summary totals) and *temporal* per-blob anomalies. Neither requires content_hash. Both require multiple epochs. **Residual**: serve endpoint selection pattern observation, key envelope size (~6KB for K=10), nonce grinding at PoW cost (#29 residual), serve+mint collusion at bootstrap (#31 residual).
...
Client designates exactly one mint as the **verifying mint** (VM) per deposit session. VM receives shards, verifies integrity, generates M, escrows to relays, publishes fund confirmation. All other mints are **secondary** (SM) — sats-only deposits, confirm after observing VM's confirmation and verifying relay-escrowed M. Escrow-before-confirmation atomicity: VM MUST escrow M before publishing confirmation. All failure modes resolve via HTLC timeouts — no stuck states. Re-funding (`role=additional`) reuses existing M. See Fund Confirmation §1 for full flow, tag schema, and HTLC timeout chain (T_vm=5min, T_sm=10min). **Residual**: client relay hints for SM subscription scope, `role=additional` detection depends on client relay freshness.
...
| M6 | Issue delivery token with biased store selection | ⊘ | [D] Three-layer verification (see Glossary: Delivery token): client checks key envelope proofs, settler audits routing, store detects aggregate anomaly via routing_root. | [C] Client publishes evidence. Settler flags. Stores exit on aggregate signal. |
...
| # | Action | Type | Detection | Consequence |
|---|--------|------|-----------|-------------|
| M24 | Share correct mappings + threshold shares via authenticated gossip | ⊕ | — | Cross-mint discovery + PSS coordination works. |
| M25 | Share wrong mappings / poisoned threshold shares | ⊘ | [D] Mappings verified against relay-encrypted canonical. Threshold shares verified via VSS commitments (Feldman). | [C] Best-effort gossip degrades to relay fallback. Invalid shares rejected by VSS check. |
| M26 | Withhold mappings / shares from gossip | ⊘ | [D] Other mints fall through to relay-encrypted mappings + relay-escrowed shares | [C] Self-harming — relay layer is canonical. |
