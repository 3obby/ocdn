...
const MIN_POW_POST = Number(process.env.NOSTR_MIN_POW_POST ?? "8");
const MIN_POW_BOOST = Number(process.env.NOSTR_MIN_POW_BOOST ?? "12");
const BOOST_TTL_EXTENSION_MS = 30 * 60 * 1000;
...
async function ingestNostrEvent(prisma: PrismaClient, event: NostrEvent) {
  const minPow = event.kind === 1 ? MIN_POW_POST : MIN_POW_BOOST;

  const powDifficulty = countLeadingZeroBits(event.id);
  if (powDifficulty < minPow) return;
  ...
  if (event.kind === 1) {
    ...
    const bonusMs = Math.max(0, powDifficulty - MIN_POW_POST) * 60 * 60 * 1000;
    const expiresAt = new Date(event.created_at * 1000 + Math.min(baseTtlMs + bonusMs, 7 * 24 * 60 * 60 * 1000));

    const weight = BigInt(1) << BigInt(powDifficulty);
        ...
        await prisma.ephemeralPost.update({
          ...
          data: {
            ...
            expiresAt: new Date(Math.min(
              target.expiresAt.getTime() + BOOST_TTL_EXTENSION_MS,
